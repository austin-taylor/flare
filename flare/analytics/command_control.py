...
class elasticBeacon(object):
    ...
    def __init__(self,
        ...
        try:
            ...
            if self.auth == "None":
                self.es = Elasticsearch(f"{self.es_host}:{self.es_port}", timeout=self.es_timeout, verify_certs=False, use_ssl=self.use_ssl)
            else:
                self.es = Elasticsearch(f"{self.es_host}:{self.es_port}", timeout=self.es_timeout, http_auth=(self.auth_user, self.auth_password), verify_certs=False, use_ssl=self.use_ssl)
